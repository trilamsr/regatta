...
1. Align with principles in the repo — PRINCIPLES.md,
 STYLE.md, ``.
2. Satisfy the initial rubric (` "### M<X>."` when the
...
Initial rubric: ` "### M<X>."` when the branch maps to
...
- Each finding has an action: `applied <SHA>`,
 ...
 PRINCIPLES.md / STYLE.md slug citation).
...
`<PR#>` and `<RUBRIC ANCHOR>` (either ` "### M<X>."`
with the resolved milestone, or `(no rubric anchor — milestone
absent; rely on PRINCIPLES.md / STYLE.md only)`) before dispatch.
 ...
 > - Standards: PRINCIPLES.md, STYLE.md,
 > 
 >
...
Apply regatta rules in full. Per
