...
3. **Dashboard provisioning** — `make provision-dashboards` calls the Grafana HTTP API to upsert every JSON file under `dashboards/grafana/`. Document the GRAFANA_URL + GRAFANA_API_TOKEN env vars; document the per-PR dashboard-JSON drift gate (`TestDashboardMetricNames_MatchEmitted`, spec §9 R2).
...
- **B (floor):** `make check` clean; `scripts/doc-check.sh` clean (link integrity + banned-phrase gates); doc ≤ 350 lines; 6 sections present; B4+B5 from spec §8.
...
- [planned] c1: `docs/operator/observability-metrics.md` checked in with the 6 mandatory sections; ≤ 350 lines (spec §6.4 + §10 dispatch brief A-T6).
