...
Add a panel to `docs/operator/dashboards/per-dag-cost.json` (file owned by A-T1; coordinate edit ordering): stat panel "Total cost USD (since boot)" — `regatta_pr_cost_usd_total`.
...
- [planned] c4: `docs/operator/dashboards/per-dag-cost.json` extended with total-cost stat panel (spec §9 R2).
