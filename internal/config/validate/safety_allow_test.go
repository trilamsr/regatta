...
// TestDestructiveOpLists asserts the resolved deny+allow surface for brief injection, nil-safe on absent config (MAY-258).
func TestDestructiveOpLists(t *testing.T) {
	var nilCfg *Config
	if deny, allow := nilCfg.DestructiveOpLists(); deny != nil || allow != nil {
		t.Fatalf("nil Config should yield nil lists; got deny=%v allow=%v", deny, allow)
	}
	cfg, err := LoadConfig([]byte(minimalValid))
	if err != nil {
		t.Fatalf("LoadConfig: %v", err)
	}
	_, allow := cfg.DestructiveOpLists()
	if len(allow) == 0 {
		t.Fatal("expected default allowlist to surface via DestructiveOpLists")
	}
}
