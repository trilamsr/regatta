...
// LoggerProvider at construction time. cmd/regatta MUST sequence
// Setup → SetLoggerProvider → NewBridgeHandler → slog.SetDefault, or
// the bridge captures the noop global for the process life.
package otel
...
// Nil primary → stderr text handler (callers wiring the root logger
// before Setup never nil-deref). Nil provider → global LoggerProvider
// (noop until Setup runs) — preserves spec §3.2's zero-cost-when-
...
// by contract — primary serialises internally; otelslog routes through
// the sdk/log Processor chain.
type bridgeHandler struct {
	primary slog.Handler
	otel    slog.Handler
}

// Enabled OR's both legs so records survive when the primary sink is at
// Warn but the OTel backend wants Debug.
...
// Handle dispatches to both legs. The Clone is mandatory: slog.Record's
// Attrs backing array is shared and otelslog walks Attrs to translate
// kinds — without the clone it would mutate state the primary later
// observes.
