...
func (o *Orchestrator) ScheduleOnce(ctx context.Context) error {
	...
	defer o.emitTickCompleted(ctx, level, startedAt, evaluated)

	for _, id := range ids {
		o.spawnReservedAgent(ctx, id)
	}
...
// emitTickCompleted fires the tick.completed log + substrate liveness
// heartbeat + slow-tick warn. Runs deferred from ScheduleOnce so
// partial-reservation error paths still emit the completion pair
// (spec §3.3). Substrate heartbeat is load-bearing: sibling-CLI
// `regatta status` reads substrate-event mtime as the daemon-up signal;
// without this, an idle daemon (no spawn activity) emits zero substrate
// events and status falsely reports "regatta serve not detected".
// Bounded write rate at 5s tick: ~17k rows/day, comparable to
// spawn/merge event volume on a busy session.
func (o *Orchestrator) emitTickCompleted(ctx context.Context, level slog.Level, startedAt time.Time, evaluated int) {
	durationMs := o.cfg.Clock().Sub(startedAt).Milliseconds()
	// R19-A O1: an NTP backwards step between startedAt and this
	// read produces a negative duration that mis-fires the
...
// spawnReservedAgent processes one reserved agent id: load, apply
// backoff + brief-body gates, invoke the spawner, and stamp the
// resulting PID+session onto the agent row. All error branches
// continue the outer tick loop — a bad agent must not abort siblings
// (R19-A O2).
func (o *Orchestrator) spawnReservedAgent(ctx context.Context, id int64) {
	...
	o.stampPostSpawn(ctx, a, result.PID, result.SessionID)
}

// stampPostSpawn moves a spawning agent to running (or, if the
// transition fails, to crashed) with its PID + SessionID durably
// attached. Order-dependent: on transition failure, the crashed stamp
// MUST land before ReleaseAgentLocks — R19-A O3 requires the PID to
// stay reachable for the reaper even after the lane lock is freed,
// otherwise ListSpawnable's `a.id IS NULL` filter would let the
// scheduler start a SECOND process for the same work_item while the
// first PID still runs.
func (o *Orchestrator) stampPostSpawn(ctx context.Context, a *state.Agent, pid int, sess string) {
