...
func defaultPromptBuilder(req Request) string {
	...
	b.WriteString("- Pre-push `make check` MANDATORY (relaxed from per-commit per MAY-276): run `make check` exactly ONCE before `git push` and verify exit=0. Use the PIPESTATUS-safe form `set -o pipefail; make check 2>&1 | tee /tmp/cicheck.log | grep -E '^(FAIL|ok|---|Error|error:|PASS)' | tail -40; echo \"exit=$?\"` — naive `cmd | tee | grep | tail; echo $?` samples TAIL's exit, not make's (false green). During iteration use `go test ./<changed-pkg>/... -short` (~5s) as the cheaper signal. Never push broken state. Per CLAUDE.md feedback_pre_commit_make_check + feedback_subagent_cicheck_compress.\n")
	b.WriteString("- Fast precheck before `make check`: on every iteration run `go build ./... && go vet ./<changed-pkg>/...` FIRST (~5s); only escalate to `make check` (~60s) once build+vet are green. Fail-fast on syntax/typos at 1/12th the cost. Per CLAUDE.md feedback_compile_precheck.\n")
	b.WriteString("- No `while pgrep / sleep` babysit loops on long-running commands: run `make check` in the FOREGROUND or use the harness Monitor tool. Chained `until ! pgrep ...; do sleep N; done` patterns burn 30-50min idle per implementer because the model wakes only after each sleep elapses. Per CLAUDE.md feedback_no_pgrep_sleep_babysit.\n")
	b.WriteString("- Grep first, Read targeted: default to `Grep` + `Read` with `offset + limit` (max 200 lines). Full-file Read only when about to Edit AND file ≤300 lines. 45-73% of implementer Reads were unnecessary full-file pulls (audit 2026-06-21). Per CLAUDE.md feedback_grep_first_read_targeted.\n")
	...
	b.WriteString("- Audit main BEFORE building: `git ls-tree -r origin/main --name-only | grep <expected-path>` OR `git log --oneline origin/main | grep '(#<issue>)'`; self-abort + report if the target is already shipped. Per CLAUDE.md feedback_audit_main_before_implementing.\n")
	b.WriteString("- Verify premise BEFORE deletion: `git grep <basename> -- . ':!.gitignore'` PLUS check docker-compose*/Dockerfile*/*.yaml mounts + CI configs. A gitignored-BUT-tracked file (`git ls-files <path>` non-empty) is deliberate infra, NOT junk — do not delete it as cruft. Per CLAUDE.md feedback_premise_before_deletion.\n")
	b.WriteString("- Verify before acting: investigator/reviewer output is a LEAD, not ground truth; spot-check 2-3 cited file:line refs + run a 1-min local measurement before recommending CI/perf/memory changes. Per CLAUDE.md feedback_validate_before_ship, feedback_subagent_output_verify.\n")
	b.WriteString("- Honest TDD ordering: land the failing test in a SEPARATE commit BEFORE impl so `git log --reverse` shows red->green; never claim red-first when test+impl are one commit — state the truth + justify single-commit. Per CLAUDE.md feedback_honest_tdd_claims.\n")
	b.WriteString("- STOP at `gh pr ready`: after pushing + opening PR + running `gh pr ready <N>`, STOP. No self-revise commits, no pre-emptive fixups, no further pushes. Wait for explicit feedback from operator-dispatched independent reviewer OR operator. Self-revising races operator reviewer dispatch (stale PR head). Per CLAUDE.md feedback_stop_at_pr_ready.\n")
	b.WriteString("- Explicit completion signal: at end of session (after `gh pr ready <N>` + report block printed), emit ONE final Bash call: `echo \"AGENT_DONE: implementer pr=<N> branch=<branch> sha=<sha>\"`. Silent-ends (no final tool call) leave the harness uncertain whether the agent is still running — operator wastes time polling transcript-mtime. Per CLAUDE.md feedback_explicit_completion_signal.\n")
