...
func TestCascadeArchiveChildrenAt_UsesProvidedTimestamp(t *testing.T) {
	db := newWorkItemsTestDB(t)
	wallClock := time.Date(1999, 1, 1, 0, 0, 0, 0, time.UTC)
	db.SetClock(func() time.Time { return wallClock })
	ctx := context.Background()
	...
	if err := db.UpsertWorkItemAt(ctx, parent, SourceAdapter, at); err != nil {
		t.Fatal(err)
	}
	for _, id := range []string{"F-AT-1", "F-AT-2"} {
		...
		if err := db.UpsertWorkItemAt(ctx, child, SourceBrief, at); err != nil {
			t.Fatal(err)
		}
	}

	cascadeAt := at.Add(time.Hour)
	archived, err := db.CascadeArchiveChildrenAt(ctx, "PROG-AT", cascadeAt)
	if err != nil {
		t.Fatalf("CascadeArchiveChildrenAt: %v", err)
	}
	if len(archived) != 2 {
	...
	for _, id := range archived {
		...
		if got.UpdatedAt.Unix() != cascadeAt.Unix() {
			t.Fatalf("%s.UpdatedAt=%d want %d (must use caller-supplied at, not d.now)", id, got.UpdatedAt.Unix(), cascadeAt.Unix())
		}
		if got.Status != WorkStatusArchived {
...
func TestCascadeArchiveChildrenAt_ReturnsNewlyArchivedIDs(t *testing.T) {
	db := newWorkItemsTestDB(t)
	t0 := time.Date(2026, 5, 30, 12, 0, 0, 0, time.UTC)
	db.SetClock(func() time.Time { return t0 })
	ctx := context.Background()
	...
	if err := db.UpsertWorkItemAt(ctx, parent, SourceAdapter, t0); err != nil {
		t.Fatal(err)
	}
	...
	for _, id := range live {
		...
		if err := db.UpsertWorkItemAt(ctx, c, SourceBrief, t0); err != nil {
			t.Fatal(err)
		}
	}
	already := WorkItem{ID: "F-N-DONE", Kind: KindFeature, Title: "done", Lane: "server",
		Status: WorkStatusArchived, ParentProgramID: "PROG-N"}
	if err := db.UpsertWorkItemAt(ctx, already, SourceBrief, t0); err != nil {
		t.Fatal(err)
	}

	archived, err := db.CascadeArchiveChildrenAt(ctx, "PROG-N", t0.Add(time.Minute))
	if err != nil {
		t.Fatalf("CascadeArchiveChildrenAt: %v", err)
	}
	if len(archived) != 2 {
...
func TestCascadeArchiveChildren_DelegatesToAt(t *testing.T) {
	db := newWorkItemsTestDB(t)
	t0 := time.Date(2026, 5, 30, 12, 0, 0, 0, time.UTC)
	db.SetClock(func() time.Time { return t0 })
	ctx := context.Background()

	parent := WorkItem{ID: "PROG-D", Kind: KindProgram, Title: "p", Lane: "server", Status: WorkStatusPlanned}
	if err := db.UpsertWorkItem(ctx, parent, SourceAdapter); err != nil {
		t.Fatal(err)
	}
	child := WorkItem{ID: "F-D-1", Kind: KindFeature, Title: "c", Lane: "server",
		Status: WorkStatusRunning, ParentProgramID: "PROG-D"}
	if err := db.UpsertWorkItem(ctx, child, SourceBrief); err != nil {
		t.Fatal(err)
	}

	if err := db.CascadeArchiveChildren(ctx, "PROG-D"); err != nil {
		t.Fatalf("CascadeArchiveChildren: %v", err)
	}

	got, _ := db.GetWorkItem(ctx, "F-D-1")
	if got.Status != WorkStatusArchived {
		t.Fatalf("F-D-1.status=%s want archived (delegation must still flip)", got.Status)
	}
	if got.UpdatedAt.Unix() != t0.Unix() {
		t.Fatalf("F-D-1.UpdatedAt=%d want %d (delegation must thread d.now through *At)", got.UpdatedAt.Unix(), t0.Unix())
	}
}
