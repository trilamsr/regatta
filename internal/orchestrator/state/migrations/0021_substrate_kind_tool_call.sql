...
-- Operator-console S0: extend substrate_events.kind CHECK to include
-- 'tool_call'. The Claude shim emits one row per tool execution
-- carrying {agent_id, signature, declared_effect_class, observed_effect}
-- for the surprise detector (spec §3.2 + §3.7). Producer wiring lands
-- downstream; this migration is the schema bridge.
--
-- sqlite CHECK widening requires a full table-rewrite — mirror the
-- pattern from 0015 (pr_stage_transition), 0016 (green-clock kinds),
-- 0017 (cost-cap kinds). Indexes, UNIQUE constraint, and column-set
-- preserved verbatim from 0017's output schema.
...
-- +goose Down
-- +goose StatementBegin
-- Forward-only: rolling back would drop any tool_call audit rows.
SELECT 1;
-- +goose StatementEnd
