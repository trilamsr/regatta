...
.PHONY: help check ci-check ci pre-push-check scorecard-check
...
check: doc-check doc-check-test specs-index-check specs-index-test prose-dup check-memory-citations check-memory-citations-test check-phase-x-leak check-phase-x-leak-test check-tbd check-tbd-test check-scorecard-test check-comment-density check-comment-density-test check-no-bare-sleep check-no-bare-sleep-test check-state-tier-order check-state-tier-order-test lint tidy-check mod-verify verify-vendored-assets go-check property-test slo-compile-test  ## Local gate; <60s. `vet` dropped — golangci-lint enables govet (.golangci.yml). Run ad-hoc via `make vet`.
...
BODY_FILE ?= /tmp/pr-$(shell git branch --show-current).md

scorecard-check:  ## Pre-PR scorecard citation gate. Runs check-scorecard.sh on $(BODY_FILE) if present; no-op when file absent (closes #778).
	@if [ -f "$(BODY_FILE)" ]; then \
		bash scripts/check-scorecard.sh --body-file "$(BODY_FILE)"; \
	else \
		echo "scorecard-check: $(BODY_FILE) absent — skip (no draft PR body to validate)"; \
	fi

pre-push-check: check scorecard-check  ## Local pre-push gate. Runs `make check` + scorecard pre-validation + PR-body release-notes block sanity check.
