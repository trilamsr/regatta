...
| **MVP-3 Wave 1 (W6 OTel — shipped partial)** | Emit OTel traces from orchestrator + spawner + scheduler. Wave 1 partial shipped (T1 SDK setup #172, T2 slog→OTel bridge #169, A4 scheduler tracer wiring #168). | Skip the Wave 1 finish — T3 trace_id migration + T4 stream-json GenAI parser + T5 Tracer injection are required before W6 is operator-visible. |
| **MVP-3 Wave 1 (substrate v2 — events log only)** | Append events to `substrate_events` (Wave 1 ships events log only; policies primitive deferred to W8 RBAC wave; blobs primitive deferred to W11 blackboard wave). Phased read-side cutover (Phase A reads legacy, Phase B reads both flag-gated for shadow assertion, Phase C reads substrate-only after cutover-by-flag); NO mid-tx dual-write. Ships AFTER W6 T1+T2+T5 merge (W6 owns migration 0005 `trace_id` columns; substrate takes migration 0006). 3 file-disjoint tasks. Spec: `engineer/specs/phase-x/2026-06-01-unified-substrate-design.md`. | Ship before W6 T1+T2+T5 merge. Cut over to substrate-only reads before Phase B shadow assertion proves stable. Roll a CAS GC cron or policies table in Wave 1. |
| **MVP-3 Wave 2 (W7 operator web UI v2)** | Operator clicks per-approver Slack/email URL → server-rendered approval page with diff + cost panel + Approve/Reject. Read-only DAG view + cost panel per `run_id` via htmx polling. Single binary + embed.FS + self-hosted Tailwind; no JS framework, no bundler. 14 tasks across 4 waves (W7.0 listener prereq + 3 build waves). Spec: `engineer/specs/2026-06-01-w7-operator-web-ui-design.md`. | Author workflows in the UI. Use any write path beyond approve/reject. Multi-tenant / SSO / OIDC (deferred to W8). |
...
| **MVP-3 W1 (substrate)** | Unified substrate v2 — events log only (policies deferred to W8, blobs deferred to W11). Single append-only signed `substrate_events` table + one `CELDecider` consumer; collapses bespoke history tables proposed by cost-governor / approval-gates / conditional-dag / blackboard into one primitive. Ships Wave 1 AFTER W6 T1+T2+T5 (W6 owns migration 0005 trace_id columns; substrate takes 0006). 3 file-disjoint tasks. Phased read-side cutover (Phase A → B → C), no atomic dual-write. Spec: `engineer/specs/phase-x/2026-06-01-unified-substrate-design.md`. RFC-0002 (`rfcs/0002-mvp-2-conditional-dag.md`) and RFC-0003 (`rfcs/0003-mvp-2-approval-gates.md`) cite the MVP-2 history-table shapes the substrate collapses. | ~1.5k | `substrate_events` + per-`kind` payload schemas under `contracts/schemas/substrate/` (forward-fit columns `blob_digest`, `schema_version` reserved for W11) | Migration 0006 applied; Phase B shadow-assertion zero divergence over 200 ticks; p95 fold ≤ 10ms on 10M-row synthetic load |
...
| **P2.5** | Temporal OSS migration via `DurableHistory` adapter (W9 option C hybrid; substrate-default impl + Temporal-backed impl behind the same interface) | sqlite write contention >5% of scheduler ticks blocked on `database is locked` over 24h OR ≥30 concurrent programs at steady state OR replay-recovery time >60s on a single run with ≥1000 journal entries (any one, two consecutive 24h windows). Red-team rationale: [`2026-06-01-w9-temporal-vs-bespoke-redteam.md`](engineer/specs/phase-x/2026-06-01-w9-temporal-vs-bespoke-redteam.md). | Program state machine + crash recovery off the daemon; substrate impl stays read-accessible (no destructive migration); existing 3-table model fits Temporal workflow history |
...
| **P3.8** | Swap-out adapter contracts — 5 adapters (OTel exporter, OPA RBAC, Sigstore signer, Stripe metered billing, LLM gateway) behind `internal/adapters/<name>/` registration pattern. Spec: [`2026-06-01-adapter-contracts-design.md`](engineer/specs/phase-x/2026-06-01-adapter-contracts-design.md). | First customer ask for any hosted backend: OTel collector / OPA bundle server / Sigstore Fulcio+Rekor / Stripe metered billing / LLM gateway (Helicone, Portkey, LiteLLM) |
...
`./wedges/README.md` and clear the validation
...
- `./wedges/cost-governor.md` — P8.
- `./wedges/approval-gates.md` — P2, P3 (accepted decision: `rfcs/0003-mvp-2-approval-gates.md`).
- `./wedges/plan-as-code.md` — P3, P10.
- `./wedges/conditional-dag.md` — P1 (accepted decision: `rfcs/0002-mvp-2-conditional-dag.md`).
- `./wedges/blackboard.md` — P6, P9.
- `./wedges/unified-substrate.md` — P3, P9, P10 (collapses bespoke tables; see `engineer/specs/phase-x/2026-06-01-unified-substrate-design.md`).
