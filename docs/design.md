...
| **P3.8** | Swap-out adapter contracts — 5 adapters (OTel exporter, OPA RBAC, Sigstore signer, Stripe metered billing, LLM gateway) behind `internal/adapters/<name>/` registration pattern. Spec: [`2026-06-01-adapter-contracts-design.md`](engineer/specs/2026-06-01-adapter-contracts-design.md). | First customer ask for any hosted backend: OTel collector / OPA bundle server / Sigstore Fulcio+Rekor / Stripe metered billing / LLM gateway (Helicone, Portkey, LiteLLM) |
