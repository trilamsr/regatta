...
5. TDD trace — verify failing-test-first commit ordering per `feedback_tdd_discipline`. Run `git log --reverse origin/main..HEAD --name-only`; for every newly-added prod `.go` paired with a co-located new `_test.go`, the test file's first-add commit MUST precede the prod file's. REJECT when test+impl share ONE commit (false red-first) or impl lands first, absent `<!-- tdd-single-commit-justified: <reason ≥4 chars> -->` in the body. Mechanically enforced by `scripts/check-tdd-redfirst.sh` (MAY-261) — re-run it, do not trust the body's "red-first" claim.
