...
5. TDD trace — verify failing-test-first commit ordering per `feedback_tdd_discipline`. `scripts/check-tdd-redfirst.sh` (MAY-261) is the authoritative CI gate: for every newly-added prod `.go` paired with a co-located new `_test.go`, it asserts the test's first-add commit precedes the impl's, and REJECTs test+impl sharing ONE commit (false red-first) or impl-first, absent `<!-- tdd-single-commit-justified: <reason ≥4 chars> -->` in the body. Reviewer backstop (CI green is ~90% trustworthy per `feedback_subagent_verification`): re-run `git log --reverse origin/main..HEAD --name-only` locally and confirm the ordering rather than trusting the body's "red-first" prose claim.
