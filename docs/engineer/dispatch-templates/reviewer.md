...
1. `Reviewer-recommendation:` or `Reviewer-agent-id:` tokens in commit messages instead of the PR body. Tokens belong in the PR BODY (honor-system since the mechanical gate was culled 2026-07-08). Per `feedback_no_self_tagged_approve`.
2. `gh pr merge --auto` enabled by the implementer. Authors must end with `gh pr ready <N>` + operator-merge handoff. Per `feedback_no_implementer_automerge`.
...
- **Load-bearing-doc carve-out (NEVER auto-skip)** — when the diff touches any of:
  ...
  ...mandatory independent reviewer dispatch even though the mechanical gate was culled 2026-07-08. `[DOCS]` release-notes on these paths does NOT waive the independent review — honor-system, adversarial-review-every-step. Per `feedback_adversarial_review_every_step`.
...
5. TDD trace — verify failing-test-first commit ordering per `feedback_tdd_discipline`. Reviewer re-runs `git log --reverse origin/main..HEAD --name-only` and confirms the ordering rather than trusting the body's "red-first" prose claim; flag test+impl in a single commit as a HIGH finding unless the PR body carries `<!-- tdd-single-commit-justified: <reason ≥4 chars> -->`.
