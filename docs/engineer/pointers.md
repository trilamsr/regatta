...
- Worker-side comment-density directive: `internal/orchestrator/spawner/claude.go::defaultPromptBuilder` carries the `COMMENTS: zero by default.` block (regression-pinned by `TestDefaultPromptBuilder_CommentsZeroByDefaultDirective`). Mirrors implementer.md §Comments. Reviewer lens 9 catches drift.
