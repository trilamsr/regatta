# regatta — research-mode vision brief (Phase X candidate)

_Author: design subagent, 2026-06-01. Source-of-truth: `docs/wedges/research-mode.md` (thesis) + `docs/engineer/specs/2026-06-01-research-mode-extension-design.md` (locked design) + adversarial review of original 6-component synthesis (70% cut by reviewer subagent). Phase-X framing per `docs/engineer/briefs/2026-06-01-self-host-first.md`._

## 1. Where regatta IS, post-self-host (the assumed pre-condition)

The self-host-first roadmap (`2026-06-01-self-host-first.md`) ships Phases S1 → S2 → S3 in ~5-7 calendar weeks before any Phase X wedge opens. At the end of Phase S3:

- Substrate Wave 1 + Phase S3-T2 cutover: `substrate_events` is the signed audit-replay journal for cost-gov + approvals (everything-else cutover remains deferred to Phase X per self-host-first §3).
- W6 OTel backbone shipped: traces correlate across LLM spans, gate decisions, scheduler ticks.
- W8 slim authorizer shipped: `Authorizer` interface + Rego hot-reload, single-tenant default. Multi-tenant `tenant_id` propagation remains Phase X.
- W9 substrate-default `DurableHistory` impl shipped: replay + diff at substrate scale. Temporal-backed variant remains Phase X.
- Adversarial-reviewer-as-gate L4 shipped (Phase S2-T2).
- CLI approval flow proven for unattended dispatch.

**Phase X — still deferred at this point:** W7 htmx UI, W8 multi-tenant scoping, W10 Sigstore, W11 blackboard CAS, W12 billing, P3.8 swap-out adapters, W9 Temporal-backed impl. Each reopens scope only when (a) an external customer asks for it, or (b) the 30-day-green trigger fires.
...
## 2. Gap between Phase S3 and "regatta runs research"
...
## 4. Ranked wedge sequence for MVR-1 (Phase X)

### Task 0 — Schema extensions (single-file, gate-keeping)  _(Phase X MVR-1, rank #1)_
...
### Tasks 1-4 — Four methodology gates (parallel)  _(Phase X MVR-1, rank #2-#5)_
...
### Task 5 — Reproducibility cron + cost-gov wiring  _(Phase X MVR-1, rank #6)_
...
### Task 6 — Four OPA Rego rules  _(Phase X MVR-1, rank #7)_
...
The companion brief `2026-06-01-arch-simplification-pass.md` documents the collapse pass that must precede MVR-1: delete the adapter-contracts spec; flatten the splittable single-consumer sub-package trees; pick-one between the three-way `planner.go`/`planner_v2.go`/`planner_stub.go` fork; audit the `Estimator` seam-vs-impl split during the cost-package flatten. The self-host-first brief (`2026-06-01-self-host-first.md`) supplies the gating sequence (Phase S1 → S2 → S3 ships before research-mode opens scope; Phase X wedges W10/W11/W12 remain deferred). Until both the simplification pass lands AND the Phase-X trigger fires, research-mode is forbidden to dispatch.
