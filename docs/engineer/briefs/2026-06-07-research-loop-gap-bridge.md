...
extends:
  ...
  - docs/engineer/briefs/2026-06-04-roadmap-reorder-self-improve-priority.md
  - docs/engineer/briefs/2026-06-04-autotuner-closed-loop-design.md
---
...
**Adopt.** `claude-mem` corpus is already in the repo plugin set; FTS5-backed; supports `memory_search` / `observation_search`. No new dep.
...
**Shape.** Implementer dispatch brief for `kind=research` gains one required step: "before forming hypothesis, query graveyard + cite N prior attempts (success / refuted / inconclusive) in the brief". Enforced as prompt-side rule in `internal/orchestrator/spawner/claude.go::defaultPromptBuilder` plus a check in the p-hack gate: missing base-rate citation → WARN; missing for confirmatory criteria → FAIL.

**Where it lands.** Amends Task 1 (`phack` gate gains one citation check) + amends Task 0 (worker-prompt parity entry).
...
**Adopt.** SPRT (Wald 1945); impl over gonum/stat distuv (https://github.com/gonum/gonum) v0.15.1, BSD-3. ~50 LoC. Bayesian early-stop is the more ambitious form; defer.
...
| 3.7 | SPRT early-stop on K=10 | Amend Task 5 | S | ~50 LoC | gonum (verify pre-existing) |

**Net:** ~530 LoC additive, one new external dep (OTel Go SDK — Apache-2.0, NoopExporter satisfies self-host), three new tasks (7, 8, 9), three amendments (Tasks 0, 1, 5). All deltas inherit the locked spec's Phase-X gate; nothing dispatches before substrate Wave 1 + W8 slim + W9 substrate-default + arch-simplification land.
...
- **OpenInference / Phoenix span clustering** — deferred per `2026-06-02-next-horizon-roadmap.md:308`; revisit at MVR-1.5-B trigger per `2026-06-04-roadmap-reorder-self-improve-priority.md` §7.
...
- **Trap-projection / inefficiency-detector pattern mining** — already covered by R6-R11 in `2026-06-04-roadmap-reorder-self-improve-priority.md`; not a research-mode concern.
...
| `docs/engineer/briefs/2026-06-04-roadmap-reorder-self-improve-priority.md` | Roadmap-shape for self-improve + autotuner | No change — meta-improvement loop is separate from research-mode loop; do not conflate |
| `docs/engineer/briefs/2026-06-04-autotuner-closed-loop-design.md` | Autotuner trust-boundary | No change — autotuner is the consumer of self-improve findings; research-mode is a separate consumer |
...
**Net docs update:** ZERO file changes today beyond this brief. Two dispatch-template amendments queued for the research-mode dispatch window. Spec amendment (§16 cross-ref) optional but recommended once this brief lands.
...
- Roadmap reorder (self-improve / autotuner): `docs/engineer/briefs/2026-06-04-roadmap-reorder-self-improve-priority.md`
- Autotuner design: `docs/engineer/briefs/2026-06-04-autotuner-closed-loop-design.md`
