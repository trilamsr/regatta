...
1. **MVP-3 Wave 1 — unified substrate finish (T-S2 + T-S3)** — T-S1 #224 SHIPPED. Dispatch T-S2 (`internal/program/cel_decider.go` — CELDecider + gate_verdict payload) + T-S3 (`tools/*` lint + adversarial/property tests) in PARALLEL — both import T-S1's substrate package; file-disjoint per `docs/engineer/plans/2026-06-01-substrate-w1-tasks.md` §3 + §4. Use the paste-ready dispatch prompts in that plan verbatim. Pin migration numbers explicitly per `feedback_migration_number_lock` (substrate added 0006; next migration = 0007).
2. **Cost-governor (P8) wedge** — moat-widening per [[wedge-roadmap-assessment]] / [[wedge-cost-governor]]. Spec landed at `docs/engineer/specs/2026-06-01-cost-governor-design.md` (5 tasks, 3 waves, pre-call deny at `scheduler.Tick` step 0.6). Wave 1 blocked on substrate W1 finish — dispatch AFTER T-S2+T-S3 land. Plan subagent first (mirror substrate W1 plan shape), then implementer dispatch.
3. **MVP-3 W7 operator web UI v2** — spec `docs/engineer/specs/2026-06-01-w7-operator-web-ui-design.md`; Temporal-UI pattern via Go embed.FS + htmx + Tailwind CDN; scope-disciplined to approval flow + read-only DAG + cost panel; **14 file-disjoint tasks across 4 waves — W7.0 listener prereq + 3 build waves**. Dispatch order: W7.0 first (single subagent), then Wave 1+2+3 each in parallel batches of ≤4 per `feedback_session_limit_dispatch`. Unblocked once substrate W1 lands (W7 reads substrate events for DAG view + cost panel).
