...
1. **MVP-3 Wave 1 — unified substrate (events log only)** — load-bearing for W7+W8+W9+W10+W11+W12. Substrate ships AFTER W6 T1+T2+T5 merge (NOT in parallel — W6 locks the events-table shape substrate writes into; substrate's schema adds `trace_id` + `span_id` columns referencing W6 conventions). Spec: `docs/engineer/specs/2026-06-01-unified-substrate-design.md`. **3 file-disjoint subagent tasks** (Wave 1 = events log only; policies primitive deferred to W8 RBAC wave; blobs primitive deferred to W11 blackboard wave). Migration is additive (events-log-as-additive-mirror, phased read-side cutover Phase A → B → C; NO mid-tx dual-write — atomic dual-write claim dropped in v2 per review verdict). Reducer strategy per `kind` hardcoded in `defaultReducer(kind)` per `feedback_spec_pattern_authority`.
2. **Cost-governor (P8) wedge** — moat-widening per [[wedge-roadmap-assessment]] / [[wedge-cost-governor]]. Build per-DAG + per-operator USD/token caps with pre-call deny + post-hoc reconciliation against Anthropic Usage API. Spec-first via design subagent citing wedge-cost-governor memory; trap pattern P8 is load-bearing for the control-plane-for-AI-labor framing. Hook points: scheduler tick (pre-call deny), spawner SupervisorLimits (cumulative), `regatta.yaml:safety.spend_cap_usd` (extend). **Reads/writes substrate `events WHERE kind='token_spend'|'budget_reconciled'`** — dispatch AFTER substrate Wave 1 lands.
3. MVP-3 W6 OTel observability backbone — umbrella #159. Wave 1 partial shipped (T1 #172 + T2 #169). Remaining: T5 Config.Tracer injection across 8 components (depends on T1 merged; A4 #168 already landed so scheduler.go is settled). Wave 2 = T3 migration 0005 trace_id columns + T4 spawner stream-json GenAI semconv parser. Wave 3 = T6 docker-compose Jaeger E2E (scaffold pre-positioned at examples/observability/docker-compose.yml via #184) + T7 operator observability doc. Spec at `docs/engineer/specs/2026-05-31-mvp-3-w6-otel-backbone.md`. W6 OTel + substrate are sequenced (W6 first), but OTel spans and substrate events are sibling primitives, not nested (per unified-substrate v2 spec §11).
...
5. MVP-3 next-level — brief at `docs/engineer/briefs/2026-05-31-mvp-3-next-level.md`. After substrate + W6 land: **W7 operator web UI v2** (spec `docs/engineer/specs/2026-06-01-w7-operator-web-ui-design.md`; Temporal-UI pattern via Go embed.FS + htmx + Tailwind CDN; scope-disciplined to approval flow + read-only DAG + cost panel; **14 file-disjoint tasks across 4 waves — W7.0 listener prereq + 3 build waves**). **W8 OPA RBAC + multi-tenant** plugs into W7's `Authorizer` interface seam (designed pre-W8 in W7 spec §3.6.4 — one-file impl swap, no re-architecture). **W9 replay+diff harness** ships AFTER W6/W7/W8 land; substrate-default `DurableHistory` impl, Temporal-backed impl gated behind refined P2.5 trigger — spec `docs/engineer/specs/2026-06-01-w9-temporal-vs-bespoke-redteam.md` picks option C (hybrid). **P3.8 swap-out adapters** (OTel exporter, OPA RBAC, Sigstore signer, Stripe billing, LLM gateway) — spec `docs/engineer/specs/2026-06-01-adapter-contracts-design.md`; trigger = first customer ask for hosted backend.
...
- **2026-06-01 design specs landed**: unified-substrate, W7 operator web UI, W9 Temporal-vs-bespoke red-team (option C hybrid picked), adapter contracts (5 swap-out)
