...
- [`2026-06-02-phase-autonomy-w2-c2-merge-execute.md`](2026-06-02-phase-autonomy-w2-c2-merge-execute.md) — PHASE AUTONOMY §11 W2 c2: wire `gh pr merge --squash --auto --delete-branch` between c0's atomic `PrepareMerge` and the `merge_completed` event. New `merge.Coordinator.ExecuteMerge` + `merge/executor` package + dedicated `mergeWorker` goroutine + `merge_executed` audit event + `regatta merge status` CLI. 11+ risks pre-addressed; no schema migration (rides c0's 0013 unique-event index).
...
- [`2026-06-02-phase-autonomy-w6-secret-credential-fetch.md`](2026-06-02-phase-autonomy-w6-secret-credential-fetch.md) — PHASE-AUTONOMY W6 secret-credential autonomic fetch: supervisor reads 4 canonical secrets (`regatta.anthropic_api_key`, `regatta.gh_token`, `regatta.brief_hmac_keys`, `regatta.audit_hmac_key`) at boot via a `Fetcher` chain — macOS Keychain (go-keyring) → `pass` (Linux) → env-var fallback. New `internal/secrets/` pkg + `regatta secret set|get|status` CLI + SIGHUP-driven cache refresh. Vault / AWS-SM / GCP-SM Phase-X behind the same interface. Ships in Landing 2 alongside W3.
