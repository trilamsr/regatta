...
**Lockfile lifecycle (closes #1006):** The handshake anchor is the LEDGER FILE itself, not a sibling `.lock`. Each daemon `os.Stat(ledger_path)` at startup to record `(dev, inode, ctime)`. The first daemon to start creates `ledger_path` via `os.OpenFile(... O_CREAT|O_EXCL ...)`; the `O_EXCL` semantic closes the ENOENT race window — exactly one daemon wins the create. Late daemons see the existing file and Stat it.
