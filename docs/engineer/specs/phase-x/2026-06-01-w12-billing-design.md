...
- **Hard prereq (merged):** Unified Substrate v2 — `docs/engineer/specs/phase-x/2026-06-01-unified-substrate-design.md`. W12 adds one new event kind `billing_period_closed` (reducer = `lww` by `tenant_id, period_start`). No DDL change — kind is a string column per substrate §2.
