...
The 2026-06-08 / 2026-06-09 dogfood sessions left a clear pattern: the operator filed every friction tracker by hand, while the orchestrator emitted the triggering signal as a structured slog event in every case. Concrete samples from this session (each filed manually by the operator after reading logs):
...
### 2.2 R6-friction — one detector rule covers the three kinds

The sibling spec `docs/engineer/specs/2026-06-08-w45-detector-rules-r6-r11.md` claims R6 for "latency-outlier" (skeleton-prefetch, baseline-gated). To avoid a numbering collision, this spec names the new rule **R6-friction** (rule registry name `friction_recurrence`). The W4.5 spec is `phase: x-prefetch` and the cited R6 is not yet implemented; if both ship, R6 is split into R6a (latency-outlier) and R6b (friction) at impl time — purely a naming decision deferred to the implementer. The rule MECHANICS in this spec are independent of the W4.5 numbering.

R6-friction reuses the existing `streakRule` primitive at `internal/selfimprove/rules.go:38-46`. Single rule registered three times (once per kind) is the simplest viable shape; one declaration per source preserves `EventKinds()` filtering and lets the operator mute individual sources via the W4 `regatta self-improvement mute <name>` surface promised in W4 §8.1.

| Sub-rule | Window | Threshold | group_by | Severity |
|---|---|---|---|---|
| `friction_recurrence_agent_exit` | 24h | 3 events same `(exit_reason, last_text_hash)` | `exit_reason`, `last_text_hash` | medium |
...
Thresholds are conservative defaults; per `feedback_default_simpler` we do NOT pre-build YAML tuning. If a default mis-fires twice in one operator session, file a tracker against the default — that is the calibration loop, not a tier system.
...
The closed_at column is opt-in housekeeping (one `gh issue view` per scan iteration, cheap) so the throttle window can distinguish "already closed → free up budget" from "still open → keep counting." If that complicates v1, drop closed_at — the throttle window is small enough (24h / 7d) that orphan rows fall off naturally.
...
```
Filed by `regatta self-improve` detector_rules **R6-friction** /
...
Operator overrides:
- Add label `do-not-auto-file:R6-friction` to THIS issue to silence
...
- **5/day**: count rows in `filed_friction_trackers` with `filed_at >= now - 24h`. If ≥5, skip and emit substrate event `friction_tracker_throttled` (carries the would-be DedupKey + the cap that tripped). The same event-kind is itself a candidate for R6-friction in future revisions, so throttle exhaustion becomes visible.
- **50/week**: same query with 7d window.

Both caps share one in-process counter rebuilt at scan start; no separate persistence. Conservative defaults per `feedback_default_simpler`; raise post-soak if the operator observes the throttle eating real signal. Lower if the auto-file rate is annoying.

### 2.6 Operator override — `do-not-auto-file:<rule-id>` label

Two override granularities, both label-driven on any OPEN issue:

- `do-not-auto-file:R6-friction` — silences the whole rule.
- `do-not-auto-file:<sub-rule-name>` — silences one sub-rule (e.g. `friction_recurrence_spawn_failed`).

The scan loop reads these via one `gh issue list --label "do-not-auto-file:R6-friction" --state open --json number -L 5` per scan iteration; presence of ANY open issue with the label short-circuits the rule. Removing the label OR closing the issue lifts the override. This is the "stop the detector while my fix is in flight" workflow per issue #1077 design consideration 5.
...
5. **Operator override** — `TestOverride_DoNotAutoFileLabel_ShortCircuitsRule` in a new `cmd/regatta/serve_friction_integration_test.go`. Uses `internal/ghclient` test fakes; asserts that the presence of an open issue with label `do-not-auto-file:R6-friction` skips ALL three sub-rules. Companion test `TestOverride_SubRuleLabel_ScopesToOneSubRule` asserts the sub-rule label only skips one sub-rule.
...
9. **No leaks across rules** — `TestThrottle_PerRuleNotShared` (R6-friction's throttle does NOT consume budget belonging to future detector rules; budget is per-rule-class if multiple ship).
...
- **Rule R7–R11 from sibling spec** — `docs/engineer/specs/2026-06-08-w45-detector-rules-r6-r11.md` is skeleton-prefetch and baseline-gated. R6-friction does NOT block on that wedge.
...
- **Cross-rule correlation** — R6-friction fires three sub-rules independently. No correlation predicate ("agent exit AND spawn fail AND tick slow on same lane → super-finding"). Add when a real session shows it would have caught a defect the per-rule fires missed.
- **Severity → priority routing** — every R6-friction tracker lands with the `state:auto-improve` label and no priority. The operator triage flow handles routing; the detector does not pick priority.
...
- **Closed-loop with autotuner (#926)** — R6-friction is operator-visibility only; it does NOT feed any autotuner knob. The sibling spec §8 ratifies which rules feed which knobs; R6-friction lands strictly outside that table.
...
- **Operator override label race.** Operator adds `do-not-auto-file:R6-friction` between the scan's label-query and the issue-create call; the override is missed for one fire. Reviewer: this race is small (one detect window) but real. Decide whether to re-check the label inside the create path or accept the one-fire slip.
- **Rule-numbering collision with W4.5.** §2.2 acknowledges this; reviewer should confirm the deferral (R6 split at impl) is acceptable to the W4.5 spec author OR that R6-friction renames to R12+ now.
...
- **PR-watch interaction.** R6-friction creates issues; do those issues appear as work_items the orchestrator picks up and dispatches an agent against? If yes, infinite loop (agent fails → R6-friction files issue → agent picks up → fails). Reviewer: confirm the `state:auto-improve` label is in the orchestrator's adapter denylist OR explicitly NOT a triggering label.
...
```
Scope: Implement R6-friction per spec §2. Land in 3 PRs:
...
Files (PR-B): internal/selfimprove/rules.go (R6-friction registration),
...
- **Reopen when** any of:
  ...
  - A new slog event-kind in the friction surface lands (`reaper_killed_repeat`, `prwatch_renamed_recurrent`, etc.) and the operator files ≥2 manual trackers for it within 7d (extend R6-friction to a 4th sub-rule).
