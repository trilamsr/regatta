...
**K5 second-key constraint (§16 #2 hardened):** autotuner MUST NOT open K5 PRs directly; instead, file a `[self-improvement]` issue carrying the candidate token + R2 evidence, then open the K5 PR only after the issue picks up the `autotune-k5-approved` label. Label comparison is BYTE-EQUAL lowercase (`label == "autotune-k5-approved"` — NOT `strings.EqualFold`). Variants like `Autotune-K5-Approved` do NOT satisfy the gate.
