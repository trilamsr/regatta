...
- Substrate W1 (T-S1 `AppendEvent` / `Fold` / `RegisterPayloadValidator` / sign+UNIQUE pattern to reuse) — shipped in #224.
...
- Substrate W1 cross-task seam contracts (T-S1 exports `AppendEvent`, `Fold`, `RegisterPayloadValidator`, `DefaultTenantID`) — shipped in #224.
