...
| JOSE/JWE (RFC 7516)                                           | REJECTED v1                 | JWE adds JSON-in-JSON nesting (extra parse + bigger payload), header fields we don't use (alg negotiation, JKU), and bespoke base64url encoding mismatches our canonical-JSON path. Bespoke `{v,kid,ct,aad}` is 4-field, fixed-shape, embeds cleanly in our canonical-JSON. Reconsider at MVR-3 if external integrators ask for JWE compat. Auditor familiarity / `go-jose` ecosystem noted but not load-bearing for at-rest envelope. |
...
**Run-time KEK loss.** If the KEK file is deleted or rendered unreadable while the service is running, the in-process key handle remains valid until process restart. On restart, KEK load fails: service refuses to start (no fail-open). New Seal/Open ops in a running process return `ErrKEKUnavailable` only if the process re-reads the KEK; today we cache at startup, so the failure surfaces at the next service restart, not at run-time. Operator runbook: restore KEK from backup before restart.
...
**Cross-subject PII.** A single event MUST contain PII fields from a single subject_id only. Cross-subject PII (e.g. an approval event referencing both reviewer_id PII and author PII) MUST be split into two events, each sealed under its subject's DEK. Rationale: cleanly maps shred-request → event-set → DEK destruction. Multi-key envelope per field is rejected (complexity, harder audit). Enforced by CUE schema validator: events with `pii: [...]` may declare exactly one `subject_id` field.
...
**Sigstore interaction (detail).** W10 (sigstore) signs the canonical bytes of each event row, including the ciphertext envelope. Shredding destroys the DEK but does NOT modify the row, so the sigstore signature continues to verify after shred. The `[REDACTED:erasure-<date>]` marker is a renderer-side substitution at read time; sigstore Verify processes binary bytes and never unmarshals the plaintext. No special handling required in W10 impl.
...
7. `[followup] LEGAL: crypto-shredding regulatory precedent` — obtain EDPB/DPA guidance or legal-counsel memo on Art. 17 acceptance of crypto-shredding before impl PR lands. Spec assumes lawful erasure but cites no precedent. **Gates the impl PR.** Reopen: regulated customer LOI received OR external legal review available.
8. `[followup] Multi-tenant PK migration for data_keys table` — schema today is single-tenant; W8 will require PK = (tenant_id, subject_id) and RLS scoping. Avoid blocker at W8 time. Reopen: W8 multi-tenant gates merge.
