...
- [ ] `EventBinaryUpdatedDetected` is added to `internal/obs/events.go::AllEventNames()`; `events_test.go` schema test passes.
...
- `internal/obs/events.go` — add `EventBinaryUpdatedDetected` to the constants block + `AllEventNames()`.
...
- Operator runs `docker compose up -d --build regatta` ≥10×/week for ≥2 weeks → reopen sidecar section and spawn `cmd/regatta-watcher` implementer.
