...
- **c8** — Given a non-existent agent ID 999, all three verbs (`pause`, `resume`, `kill`) exit 1 with stderr `agent 999 not found`.
...
- **`kill` after the agent has organically transitioned to `done`**: c7 covers terminal states. Edge case: the transition happens BETWEEN the CLI's state read and its `TransitionAgent` call. The transition fails with `ErrInvalidTransition: done → crashed`; CLI exits 1 with stderr noting the race.
...
- EDIT `internal/orchestrator/state/agents.go::UpsertPendingTx` — add the pause-check guard inside the reservation closure (covers the mid-Tick race in Adversarial). Add a typed sentinel `ErrAgentPaused` so the scheduler can quietly skip without error-log noise.
