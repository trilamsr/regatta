...
- **OTel GenAI semconv as the wire-format for LLM spans**. regatta's W6 OTel backbone already ships GenAI semconv attrs per #213 (`gen_ai.usage.input_tokens`, `gen_ai.usage.output_tokens`, `gen_ai.request.model`). OpenInference + OpenLLMetry are sibling attribute sets that some ecosystems (Phoenix/Arize) ingest natively — bridging is an optional follow-up, NOT the default. **Action: no infra change; regatta-owned spans stay on GenAI semconv. File followup IF a Phoenix/Arize integration is requested. This is a `feedback_research_design_principles` "borrow proven OSS" win — do not invent a regatta-specific attribute schema.**
...
4. **OpenTelemetry GenAI semconv has won; OpenInference + OpenLLMetry are sibling attribute sets.** Phoenix, Langfuse, LangSmith, Laminar all ingest OTel spans. W6 OTel backbone ships GenAI semconv per #213 — that's the regatta-native attribute set; bridging to OpenInference is an integration-time concern, NOT a default. **Action: no infra change; bridging deferred to integration-time.** This is a "borrow OSS standards" win — refuse to invent a regatta-specific LLM-span schema.
...
- **Risk: load-bearing leftovers.** Per `feedback_unaddressed_load_bearing`: this brief surfaces three "verify-or-file-issue" items — (a) W6 GenAI-semconv→OpenInference bridge (integration-time only), (b) Inngest-shape `step.run` parity with `substrate_events`, (c) regatta automerge logic matching Renovate's mental model. These are research-brief observations, not implementation; the convention per `feedback_unaddressed_load_bearing` is to file follow-up issues. Recommend: open three tracking issues post-merge.
