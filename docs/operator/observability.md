...
`docs/engineer/specs/phase-x/2026-05-31-mvp-3-w6-otel-backbone.md` §3.6 changes.
...
| `not_found` | The PR (or branch, or repo) backing the agent does not exist — operator deleted the PR, branch was force-deleted, or the brief points at the wrong repo. Distinct from `absent` per issue #498 — `not_found` means the target itself is missing, not the label. | `sqlite3 .regatta/state.db "SELECT work_item_id, pr_sha FROM agents WHERE state='escalated'"` for the resolved branch; verify the repo + branch + open-PR state. Withdraw the agent if the target is gone. |
