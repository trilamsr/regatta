...
- **`make check` is the single source of truth for what is verified
  ...
  `lint`, `tidy-check`, `mod-verify`, `go-check`. Stays under 60
  seconds locally; `make ci-check` extends it with `stale-todo`.
  New gates land here, not in ad-hoc scripts. Anchor: `Makefile`
  target `check`.
