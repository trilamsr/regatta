...
- **Reviewer-verdict gate** (`scripts/check-reviewer-verdict.sh`, wired into pr-lint workflow): load-bearing PRs (paths under `internal/{ghclient,gates,orchestrator,supervisor,ghidempotency,secrets,sandbox}/`, `cmd/`, `contracts/schemas/`, PLUS agent-rule + CI-gate surfaces `CLAUDE.md`, `Makefile`, `Makefile.d/*`, `.github/workflows/*`, `docs/engineer/dispatch-templates/*`, `scripts/check-*.sh`, PLUS load-bearing-doc surfaces `docs/engineer/specs/*.md`, `docs/engineer/briefs/*.md`) MUST carry `Reviewer-recommendation: APPROVE` in the PR body footer (bare, not in a code block). Missing → fail. `REVISE`/`BLOCK` → fail. `[CHORE]`/`[DOCS]`/`[CI]`/`[NONE]`/`[CHANGE]` release-notes auto-skips — EXCEPT when path classifier flags an agent-rule / CI-gate / load-bearing-doc surface (those surfaces are themselves the category being reviewed). Closes session retro 2026-06-04 Impact 1 (#899) + retro 2026-06-08 (#986 #991).
...
- **No self-tagged `Reviewer-recommendation: APPROVE`**: author writing own APPROVE token == zero adversarial pass; the gate passes mechanically while no review happened. Spawn `cavecrew-reviewer` (or equivalent independent agent) in fresh slot BEFORE the token lands; if reviewer finds HIGH/MED, fix inline OR file tracking issue + cite # before writing APPROVE. 2026-06-08 session: PRs #999/#1001/#1002 self-tagged → retro adversarial pass found 1 HIGH + 3 MED (filed as #1004 #1005 #1006 #1007). (`feedback_no_self_tagged_approve`)
