...
- **Scorecard evidence-token shape**: every `[x]` in A+ scorecard MUST cite ONE of: `Test*`-name / `path.ext:NN` / `#NNN` / `N/A — <reason>` ON THE SAME LINE. Prose-only explanation always fails `scripts/check-scorecard.sh:171-187` (regex requires extension dot + colon + digit OR `#` + digit OR literal `N/A`). Memory slugs (`feedback_*`) and repo handles (`org/repo`) DO NOT match. Category auto-skip [CHORE]/[DOCS]/[CI]/[NONE]/[CHANGE]/[REFACTOR] only when NO scorecard section exists; partial scorecards still gate. (`feedback_scorecard_evidence_token_required`)
...
- **Scorecard row labels required** — every scorecard row MUST open with a criterion label: `(a)`/`(b)`/`(c)`-form lowercase parens OR `B1`/`B2`/`A1`/`A2`/`A+1`/`A+2`-form tier-letter+digit. Without a label, `scripts/check-scorecard.sh` uncited-row errors emit `criteria (unlabeled)` and operators must count rows to find offenders. Canonical copy-paste block lives in `docs/engineer/dispatch-templates/implementer.md` §A+ SCORECARD. (#855, `feedback_grade_rubric`)
