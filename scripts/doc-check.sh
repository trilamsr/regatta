...
# Specs-index is no longer tracked in git (docs/engineer/specs/README.md
# is gitignored; regenerate locally with `make specs-index`). The
# cascade-rebase storm this gate used to close is now closed by
# untracking the artifact entirely — no shared anchor, no drift to
# detect.
